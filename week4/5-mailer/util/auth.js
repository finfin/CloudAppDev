...
exports.ensureAuthenticated = ensureAuthenticated = function(req, res, next) {
    if (req.isAuthenticated() ) { //TODO: check user enabled
        return next();
    }
...
exports.ensureAdmin = ensureAdmin = function(req, res, next) {
    if (req.user && req.user.admin === true) { //TODO: check user enabled
        return next();
    } else {
        return res.send(403);
    }
...
exports.ensureSelfOrAdmin = function(req, res, next) {
    ...
    if ( req.user && (req.user.admin === true || req.user.name === name) ) { //TODO: check user enabled
        return next();
    } else {
        return res.send(403);
    }
